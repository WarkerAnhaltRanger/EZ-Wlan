...
public class ArcadyanWlanKeyHandler extends AbstractWlanKeyHandler {
    ...
    protected String calcKey(String bssid){
        // code from Wotan.cc
        String wpaKey = "";
        String mac = bssid;
        ...
        String tmpK1 = Integer.toHexString(Character.digit(S7, 16) +
                Character.digit(S8, 16) +
                Character.digit(M11, 16) +
                Character.digit(M12, 16));

        String tmpK2 = Integer.toHexString(Character.digit(M9, 16) +
	...
	public String[] getKeys(ScanResult sr) {
        return new String[] { calcKey(HandlerFactory.bssid_helper(sr.BSSID)) };
	}
